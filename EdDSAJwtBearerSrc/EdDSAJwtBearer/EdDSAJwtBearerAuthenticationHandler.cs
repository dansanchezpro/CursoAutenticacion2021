...
namespace EdDSAJwtBearer
{
    ...
    {
        public EdDSAJwtBearerAuthenticationHandler(IOptionsMonitor<EdDSAJwtBearerOptions> options, ILoggerFactory logger,
                ...
                if (AuthenticationHeaderValue.TryParse(Request.Headers["Authorization"], out AuthenticationHeaderValue HeaderValue))
                {
                    if ("Bearer".Equals(HeaderValue.Scheme, StringComparison.OrdinalIgnoreCase))
                    {
                        try
                        {
                            ...
                            {
                                List<Claim> Claims = Payload.Where(c => c.Key != "roles").Select(c => new Claim(c.Key, $"{c.Value}")).ToList();
                                ...
                                ClaimsIdentity Identity = new ClaimsIdentity(Claims, Scheme.Name, "firstName", "role");
                                ClaimsPrincipal Principal = new ClaimsPrincipal(Identity);

                                AuthenticationTicket Ticket;
                                if (Options.SaveToken)
                                {
                                    //Almacenar el token en una instancea de AuthenticationProperties
                                    var Properties = new AuthenticationProperties();
                                    Properties.StoreTokens(new AuthenticationToken[] { new AuthenticationToken { Name = "access_token", Value = Token } });
        ...
        private bool TryGetPayloadWithTokenValidation(string token, EdDSAJwtBearerOptions options, out Dictionary<string, object> payload, out string error)
        {
            bool IsValid;
            payload = default;
            error = string.Empty;
            ...
            try
            {
                if (EdDSATokenHandler.TryGetPayloadFromToken(token, options.PublicSigningKey, out payload))
                {
                    ...
                    if (IsValid && options.ValidateAudience)
                    {
                        ...
                        if (IsValid)
                        {
                            string[] Audience = Value.ToString().Split(",");
                            IsValid = Audience.Contains(options.ValidAudience);
                        }
